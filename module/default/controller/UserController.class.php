...
class UserController  extends Controller
{
    ...
    public function indexAction() {
        ...
        if (file_exists($photo)) {
            ...
            $img = PUBLIC_PATH.'uploads/photo/11.jpg';
        }
    ...
    public function editAction(){        
        ...
        $userinoFile = PUBLIC_PATH.'tmp/userinfo/'.$user['name'].'.txt';
        //数据初始化
        $role = array('网络空间安全','网络空间安全实验班','信息安全','网络工程','密码学');
        $gender = array('男','女');
        //有表单提交时，接收表单数据并输出
        if($_POST){
            //定义需要接收的字段
            ...
            //验证完成，保存文件
            //将数组序列化为字符串
            $data = serialize($user_data);         //serialize   
            //将字符串保存到文件中 
    ...
    public function loginAction() {
        ...
        if ($_POST) {
            ...
            $dbName='ef';//使用的数据库
            $user='hrt123456';//数据库连接用户名
            $pass='hrt102600srx';//对应的密码
            ...
            $link = mysqli_connect("localhost", "hrt123456", "hrt102600srx", "ef");
            try {
                $connID = new PDO($dsn, $user, $pass);
                $query="select * from test_user where name='$name' and password='$pwd'"; 
                $stmt = $connID->prepare($query);
                ...
                if ($result) {
                    ...
                    $msg = '登录失败，用户名或密码错误';
                    header('content-type:text/html;charset=utf-8');
                    $errorMsg = "Error: " . htmlspecialchars($msg);
                    $errorMsg .= "SQL: " . htmlspecialchars($sql);
                    exit('<script>alert("' . $errorMsg . '");history.back();</script>');
                }
            } 
